...
class LearningManager:
    ...
    def readdata(self, filedir): # 학습 데이터를 읽음
    ...
    def preprocessing(self): # StandardScaler를 사용해 input 전처리
        self.sc.fit(self.x_train)
        self.x_train_std = self.sc.transform(self.x_train)
        joblib.dump(self.sc, self.sc_path)

    def resetmodel(self): # 모델 초기화
        self.classifer = SVC(random_state=1)
        self.sc = StandardScaler()

    def setmodel(self, params): # 모델 파라미터 설정
    ...
    def eval_train(self): # 학습 데이터에 대해 Cross Validation을 수행해 성능 출력
    ...
    def readtest(self, filedir):  # [content, x, y, w, h, font_size, label] 테스트 데이터를 읽음
        csv = pd.read_csv(filedir, header=None, encoding="utf-8")
        for index, row in csv.iterrows():
            if not (row[1] == 0.0 and row[2] == 0.0 and row[3] == 0.0 and row[4] == 0.0):
                block = [row[1], row[2], row[3], row[4], row[5]]
    ...
    def eval_test(self): # 테스트 데이터에 대해 성능 출력
...
learningmanager.resetmodel()
learningmanager.readdata("Dataset/preprocess/train_blog.csv")
learningmanager.preprocessing()
params = {"C": 100.0, "gamma": 0.2, "kernel": "rbf", "max_iter": 1000, "class_weight": {0: 0.2, 1: 0.8}}
learningmanager.setmodel(params)
learningmanager.fit()
learningmanager = LearningManager(4)
learningmanager.resetmodel()
learningmanager.readdata("Dataset/preprocess/train_blog_title.csv")
learningmanager.preprocessing()
params = {"C": 10.0, "gamma": 0.2, "kernel": "rbf", "max_iter": 500, "class_weight": None}
learningmanager.setmodel(params)
learningmanager.fit()
end = time.time()
print(end-start, "seconds")
...
learningmanager.resetmodel()
learningmanager.readdata("Dataset/preprocess/train_shop.csv")
learningmanager.preprocessing()
...
learningmanager.preprocessing()
params = {"C": 1.0, "gamma": 1.0, "kernel": "rbf", "max_iter": 500, "class_weight": {0: 0.2, 1: 0.8}}
learningmanager.setmodel(params)
learningmanager.fit()
end = time.time()
print(end-start, "seconds")