...
class TuningManager:
    ...
    def tuning(self, mode, params):
        ...
        pca = PCA(n_components=2) # 2차원 Decision Boundary를 그리기 위해 PCA를 사용해 5차원 데이터에서 2차원으로 압축
        sc = StandardScaler()
        ...
        plot_decision_regions(X=x, y=y, clf=self.svm, legend=2) # Decision Boundary 그리기
    ...
    def plot(self, mode, flag): # 데이터 분포 그리기
        learningmanager = self.learningmanager
        learningmanager.resetmodel()
        if flag == "train":
            if mode == 1:
